...
public class CommentServiceTest extends AbstractTest {
	...
	//		super.authenticate("admin");
	//		final int commentId = super.getEntityId("comment2");
	...
	//		super.authenticate("user2");//El que elimina un comentario es un usuario
	//		final int commentId = super.getEntityId("comment2");
	//		final Comment comment = this.commentService.findOne(commentId);
	//		this.commentService.delete(comment);
	//		Assert.isTrue(!this.commentService.findAll().contains(comment));
	...
	protected void templateDelete(final String username, final int commentId, final Class<?> expected) {
		...
		caught = null;
		try {
			super.authenticate(username);//El que elimina un comentario es un usuario
			final Comment comment = this.commentService.findOne(commentId);
			this.commentService.delete(comment);
			Assert.isTrue(!this.commentService.findAll().contains(comment));
			super.authenticate(null);
